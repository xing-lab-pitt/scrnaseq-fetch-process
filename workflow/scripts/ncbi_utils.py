...
def fetch_sra_study_accession(geo_id: str, bioproject: Optional[str] = None) -> Optional[str]:
    """
    ...
    try:
        ...
        # Not all GEO studies expose a direct SRA link; fall back to BioProject
        # (same strategy as fetch_sra_run_info / fetch_sra_run_info_detailed).
        if not id_list:
            if not bioproject:
                bioproject = fetch_bioproject_from_geo(geo_id)

            if bioproject:
                logger.info(f"Using BioProject {bioproject} to resolve SRA study for {geo_id}")
                search_url = f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=sra&term={bioproject}&retmode=json"
...
def fetch_sra_run_info_detailed(geo_id: str, bioproject: Optional[str] = None) -> List[Dict]:
    ...
    # Only GSE accessions use the [GEO] qualifier. SRA accessions (SRX/SRR/SRP)
    # and BioProject (PRJNA) resolve directly as a plain esearch term; adding
    # [GEO] to them returns nothing.
    is_gse = geo_id.upper().startswith("GSE")
    term = f"{geo_id}[GEO]" if is_gse else geo_id

    try:
        # First get SRA UIDs for the accession
        search_url = f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=sra&term={term}&retmax=1000&retmode=json"
        ...
        # If a GSE has no direct SRA link, fall back via BioProject. (SRA/
        # BioProject accessions already searched directly above, so skip.)
        if not id_list and is_gse:
